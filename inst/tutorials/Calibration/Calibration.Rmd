...
Let's start with a quiz to explore possible situations where 
you might face the decision of whether to calibrate or not
...
1. Work through the code in the box to make sure you understand
what each section is doing.

2. When you're happy hit run to perform the calibration. Remember,
calibration takes multiple model-runs so won't be instant (this
 example is artificially set up to run reasonably quickly).
 
After the calibration finishes you'll see a print out summarising
the attempted EIR and comparisons between the summarised model
output and target.
...
-   Modify the calibration code to calibrate to 3 years of
target data.
-   Read about the `weights` argument in `?calibrate()`, have
a go a weighting your target data so more recent years are more
influential.
-   What happens if you change the `tolerance` argument in 
`?calibrate()`
