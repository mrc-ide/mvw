...
2.  Create a `r emo::ji("package")` [malariasimulation] parameters list using the `site::site_parameters()` function. For the `overrides` argument you might want to set a reasonably small population so that the model runs quickly, for example `overrides  = list(human_population = 5000)`.
...
`r emo::ji("warning")` The model run may take a little while to complete (\~5 mins). While you wait, why not have a look at the [foresite website](https://mrc-ide.github.io/foresite/) `r emo::ji("warning")`
